/**
 * @brief 水中协作顶球祖传策略代码
 * @note 这代码简直是要命，调是不可能调的，一坨标签没注释
 *  根本不知道是什么意思。稍微改改整理了下，看着那么冗长的
 *  一个函数，除了结构性调整，具体策略还真是改不动。跑的实
 *  物效果也是一言难尽，所以说就让它这么放在这里吧。之后看
 *  到这里的人如果你有那么多的闲情逸致，那就试试能不能干
 *  掉它吧。
 * @author zymelaii
 * @date 2022-07-04
 */

#include "ancestralcbp.h"
#include "../mathtools.h"
#include <iostream>
...
static int flag1 = 0, flag2 = 0, flag3 = 0, flag4 = 0, flag5 = 0, flag6 = 0,
...
bool judgeareaup(CPoint point, int area_id) {
    ...
    case 5: /// 在上右平面
        return point.y < middle && point.x > width / 2;
    case 6: /// 在一门右向左区域，在靠下
        return point.y < middle && point.y > middleup - 30 &&
                     point.x < onedoorright + 20;
    case 7: /// 二球门左,右之间
    ...
    case 10: /// 一球门右以左
        return point.y < middle && point.x < onedoorright + 20;
    case 11: /// 二球门右靠下
        return point.y < middle - 50 && point.y > middleup - 30 &&
                     point.x > twodoorright;
    case 12: /// 球门区域（一二球门之间）
        return point.y < middle - 50 && point.y > middleup &&
                     point.x < twodoorright && point.x > twodoorleft;
    case 13: /// 一球门右中门以左
        return point.y < middle && point.x > onedoorright && point.x < width / 2;
    case 14: /// 中门以右二左门以左靠上
        return point.y < middleup - 30 && point.x > width / 2 &&
                     point.x < twodoorleft;
    case 15: /// 球门区域（一二球门之间）
        return point.y < middle - 50 && point.y > middleup &&
                     point.x < onedoorright && point.x > onedoorleft;
    default:
        return false;
    }
}
...
bool judgearea(CPoint point, int area_id) {
    ...
    case 2: /// 下平面
        return point.y > middle;
    case 3: /// 在下区域卡球
        return point.y > middle && point.y < middledown && point.x < width / 2 &&
                     point.x > onedoorright;
    case 7: /// 在下左平面
    ...
    case 10: /// 在下左平面(在第一门之间）
        return point.y > middledown && point.x > onedoorleft &&
                     point.x < onedoorright;
    case 11: /// 在下左平面(靠上，在第一门左面）
        return point.y > middle + 50 && point.y < middledown + 30 &&
                     point.x < onedoorleft + 10;
    case 12: /// 在下右平面(在第二门之间,middle和middledown之间)
        return point.y > middle + 50 && point.y < middledown &&
                     point.x > twodoorleft && point.x < twodoorright;
    case 13: /// 在下左平面(在第一门之间,middle和middledown之间)
        return point.y > middle && point.y < middledown &&
                     point.x > onedoorleft + 10 && point.x < onedoorright;
    case 14: /// 在下左平面(在第一门偏右,middle和middledown之间)
        return point.y > middle + 50 && point.y < middledown &&
                     point.x < onedoorright + 100 && point.x > onedoorright;
    case 15: /// 在下右平面(在第二门偏右,middle和middledown之间)
        return point.y > middle + 50 && point.y < middledown &&
                     point.x < twodoorright + 100 && point.x > twodoorright;
    case 16: /// 在下右平面(二球门左)
        return point.y > middle && point.x < twodoorright;
    case 17: /// 在下右平面(二球门右)
        return point.y > middle && point.x >= twodoorright;
    ...
    case 19: /// 在下右平面(一球门右)
        return point.y > middle && point.x >= onedoorright;
    case 20: /// 在最后门下面一点点
        return point.y > finaldoordown1 && point.y < finaldoordown &&
                     point.x > 610 && point.x < finaldoorx;
    case 21: /// 在最后门上面一条线
        return point.y < finaldoordown1 && point.y > middle &&
                     point.x > finaldoorx && point.x < finaldoorx;
    case 22: /// 在第一球门左面击球区域
        return point.y > middledown && point.x < width / 2 &&
                     point.x > onedoorright;
    case 23: /// 最后射门死区
    ...
    case 25: /// 最后球门的区域，一二球门之间靠上
        return point.y > middle + 50 && point.y < middledown &&
                     point.x < twodoorright && point.x > onedoorright;
    case 26:
        return point.y > middledown + 50 && point.x > twodoorleft &&
                     point.x < finaldoorx;
    case 27:
    ...
    case 29: /// 在下右平面(在第二门之间,middle和middledown之间)
        return point.y > middle && point.y < middledown && point.x > twodoorleft &&
                     point.x < twodoorright;
    case 30: /// 在下左平面(在第一门偏右,middle和middledown之间)
        return point.y > middle && point.y < middledown + 20 &&
                     point.x < onedoorright + 100 && point.x > onedoorright;
    case 31: /// 在下右平面(在第二门偏右,middle和middledown之间)
        return point.y > middle && point.y < middledown &&
                     point.x < twodoorright + 100 && point.x > twodoorright;
    case 32: /// 在下左平面(在第一门之间,middle和middledown之间)
        return point.y > middle && point.y < middledown && point.x > onedoorleft &&
                     point.x < onedoorright;
...
int judgeball(CPoint point) {
    ...
    if (W == 1 && (point.y > middle + 10 && point.y < middledown + 10 &&
                                 point.x > twodoorleft - 10 && point.x < twodoorright + 10)) {
        E = 1;
    }
 ...
 * @brief 垂直向上，下顶球
 */
void AncestralCBP::movingshot1(int mode, CPoint aimpt, CFishAction &action,
                                                                    int fishID, CFishInfo m_FishInfo,
                                                                    CPoint doorpt, CPoint shotpt0) {
    CPoint FishPt = m_FishInfo.centerPos();     // 鱼身坐标
    CPoint FishHeadPt = m_FishInfo.headerPos(); // 鱼头坐标
    double FishDir = normalizeAngle(getVecAngle(FishPt, FishHeadPt)) / M_PI *
                                     180; // 鱼的朝向角度（鱼身向鱼头）
    CPoint templeft0, templeft1, templeft2, tempright0, tempright1,
            tempright2; // 临时击球点
    ...
    if (mode == 0) {
        ...
        if (FishPt.x < aimpt.x && FishPt.y < aimpt.y + shopdistance + 5 &&
                FishPt.y <= 430 && FishPt.x > 40) {
            flag1 = 1;
        }
        if (FishPt.x >= aimpt.x && FishPt.y < aimpt.y + shopdistance + 5 &&
        ...
        if (flag1 == 1) {
            flag2 = 0;
            if ((FishDir > -180 && FishDir < 0) && getDistance(FishPt, aimpt) < 120 &&
                    findpoint == 0) {
    ...
    if (mode == 1) {
        if ((FishPt.y < aimpt.y - shopdistance && flag7 == 0 && flag8 == 0) ||
                FishPt.y >= 430 || FishPt.x < 40) {
            // action.speed=speed1;
            flag7 = 0;
            flag8 = 0;
        ...
        if (FishPt.x < aimpt.x && FishPt.y >= aimpt.y - shopdistance &&
                FishPt.y >= 40 && FishPt.y <= 430) {
            flag7 = 1;
        }
        if (FishPt.x > aimpt.x && FishPt.y >= aimpt.y - shopdistance &&
                FishPt.y >= 40 && FishPt.y <= 440) {
            flag8 = 1;
        }
        if (flag7 == 1) {
            flag8 = 0;
            if ((FishDir > 0 || FishDir < -150) && getDistance(FishPt, aimpt) < 120 &&
                    findpoint == 0) {
                std::cout << "右行" << std::endl;
                action.direction = 12;
        ...
        if (flag8 == 1) {
            flag7 = 0;
            if ((FishDir > -30 || FishDir < -170) &&
                    getDistance(FishPt, aimpt) < 120 && findpoint == 0) {
                std::cout << "左行" << std::endl;
                action.direction = 1;
...
void AncestralCBP::movingshot2(int mode, CPoint aimpt, CFishAction &action,
                                                                    int fishID, CFishInfo m_FishInfo,
                                                                    CPoint doorpt,
                                                                    CPoint shotpt0) //垂直向左，向右顶球
{
    ...
    double FishDir = normalizeAngle(getVecAngle(FishPt, FishHeadPt)) / M_PI *
                                     180; // 鱼的朝向角度（鱼身向鱼头）
    CPoint tempup0, tempup1, tempup2, tempdown0, tempdown1,
            tempdown2; // 临时击球点
    ...
    if (mode == 0) {
        ...
        if (flag3 == 1) {
            flag4 = 0;
            if ((FishDir < -90 || FishDir > 90) && getDistance(FishPt, aimpt) < 120 &&
                    findpoint == 0) {
                std::cout << "左行" << std::endl;
                action.direction = 1;
            } else {
        ...
        if (flag4 == 1) {
            flag3 = 0;
            if ((FishDir < -90 || FishDir > 0) && getDistance(FishPt, aimpt) < 120 &&
                    findpoint == 0) {
                std::cout << "右行" << std::endl;
                action.direction = 12;
            } else {
    ...
    if (mode == 1) {
        if ((FishPt.x < aimpt.x - shopdistance && flag5 == 0 && flag6 == 0) ||
                FishPt.x <= 80 || FishPt.x > 680) {
            flag5 = 0;
            flag6 = 0;
        ...
        if (flag5 == 1) {
            flag6 = 0;
            if ((FishDir < 90 || FishDir > 160) && getDistance(FishPt, aimpt) < 180 &&
                    findpoint == 0) {
...
void AncestralCBP::movingshot3(int mode, CPoint aimpt, CFishAction &action,
                                                                    int fishID, CFishInfo m_FishInfo,
                                                                    CPoint doorpt,
                                                                    CPoint shotpt0) { //垂直向左，向右顶球
    CPoint FishPt = m_FishInfo.centerPos();           // 鱼身坐标
    ...
    if (mode == 0) {
        if ((FishPt.x > aimpt.x + shopdistance && flag11 == 0 && flag12 == 0) ||
                FishPt.x > 680 || FishPt.x <= 80) {
            flag11 = 0;
            flag12 = 0;
            std::cout << "击球" << std::endl;
            spinP2PMove(shotpt0, m_FishInfo, action);
        }
        if (FishPt.x < aimpt.x + shopdistance && FishPt.y > aimpt.y &&
                FishPt.x <= 680) {
            flag11 = 1;
        }
        if (FishPt.x < aimpt.x + shopdistance && FishPt.y < aimpt.y &&
                FishPt.y > aimpt.y - 200 && FishPt.x <= 680) {
            flag12 = 1;
        }
        if (flag11 == 1) {
            flag12 = 0;
            if (FishDir < 180 && FishDir > 0 && getDistance(FishPt, aimpt) < 180 &&
                    findpoint == 0) {
                std::cout << "左行" << std::endl;
                action.direction = 1;
        ...
        if (flag12 == 1) {
            flag11 = 0;
            if ((FishDir < 0 || FishDir > 90) && getDistance(FishPt, aimpt) < 180 &&
                    findpoint == 0) {
                std::cout << "右行" << std::endl;
                action.direction = 12;
                // Roundp2pright(tempup0,action,0,m_FishInfo);
    ...
    if (mode == 1) {
        if ((FishPt.x < aimpt.x - shopdistance && flag9 == 0 && flag10 == 0) ||
                FishPt.x > 680 || FishPt.x <= 80) {
            flag9 = 0;
            flag10 = 0;
            std::cout << "击球" << std::endl;
        ...
        if (FishPt.x > aimpt.x - shopdistance && FishPt.y < aimpt.y - 20 &&
                FishPt.x >= 80) {
            flag9 = 1;
        }
        if (judgeball(aimpt) == 3) {
        ...
        if (flag10 == 1) {
            flag9 = 0;
            if ((FishDir > -90 || FishDir < -150) &&
                    getDistance(FishPt, aimpt) < 180 && findpoint == 0) {
                std::cout << "右行" << std::endl;
                action.direction = 12;
            } else {
                findpoint = 1;
 ...
 * @note 版本：最终版2 红4 - 改
 */
bool AncestralCBP::Strategy(RefArray<CFishAction> aAction,
    ...
    double FishDir, distfishtoaimpt, distfishtoheadpt, dirfishtoaimpt,
            dirfishtoheadpt, distballtodoor, diraimpttodoor, diraimpttodoor1,
            diraimpttodoor3, dirspeed;   // 鱼头的方向
    CPoint FishHeadPt;               //鱼头的位置
    CPoint FishTailPt;               //鱼尾巴的位置
    CPoint tempup, tempdown;         //临时击球点
    CPoint aimpt;                    //球的坐标
    CPoint doorpt, doorpt1, doorpt3; //球门坐标
    CPoint shotpt0, shotpt1, shotpt3, shotptup, shotptdown, shotptright,
            shotptleft, finalshot; //击球点坐标

    doorpt  = aChannel[0].center;
    doorpt1 = aChannel[1].center;
    doorpt3 = aChannel[2].center;

    // std::cout<<doorpt1.x<<std::endl;
    aimpt = aBallinfo[0].getCenter();
    std::cout << "水球坐标: " << "(" << aimpt.x << ", " << aimpt.y << ")\n";
    FishPt = aFish[A].centerPos();     //鱼中心点坐标
    FishPt1 = aFish[B].centerPos();    //鱼中心点坐标
    FishHeadPt = aFish[A].headerPos(); //鱼头坐标

    FishDir = normalizeAngle(getVecAngle(FishPt, FishHeadPt) * 180 / M_PI,
                                                     false); // 鱼的角度

    distfishtoaimpt = getDistance(FishPt, aimpt); //球到鱼中心的距离
    distfishtoheadpt = getDistance(FishPt, FishHeadPt); //球到鱼头的距离(有问题）

    dirfishtoheadpt = normalizeAngle(getVecAngle(aimpt, FishHeadPt)) * 180 /
                                        M_PI; //球到鱼头的角度
    dirfishtoaimpt = normalizeAngle(getVecAngle(aimpt, FishPt)) * 180 /
                                     M_PI;                       //球到鱼中心的角度
    distballtodoor = getDistance(aimpt, doorpt); //球到门的距离
    diraimpttodoor =
            normalizeAngle(getVecAngle(aimpt, doorpt)) * 180 / M_PI; //球到球门的角度
    diraimpttodoor1 =
            normalizeAngle(getVecAngle(aimpt, doorpt1)) * 180 / M_PI; //球到球门的角度
    diraimpttodoor3 =
            normalizeAngle(getVecAngle(aimpt, doorpt3)) * 180 / M_PI; //球到球门的角度
    dirspeed = fabs(normalizeAngle(FishDir - getVecAngle(FishPt, aimpt)) * 180 /
                                    M_PI); // 鱼转向角速度
    ...
    // A鱼顶左面球门策略
    std::cout << __LINE__ << "# A鱼顶左面球门策略-水球区域标号@judgeball: "
                        << judgeball(aimpt) << "\n";

    if (movement == 0) {
        std::cout << __LINE__ << "# 正在执行B鱼第一动作：向一号门正上方远距移动" << std::endl;
    ...
    if (movement == 1) {
        std::cout << __LINE__ << "# 正在执行B鱼第二动作：向一号门正下方近距移动" << std::endl;
    ...
    if (movement == 2) {
        std::cout << __LINE__ << "# 正在执行B鱼第三动作：向一号门左下侧移动" << std::endl;
    ...
    if (movement == 3) {
        std::cout << __LINE__ << "# 正在执行B鱼第四动作：向一号门正下方近距移动" << std::endl;
        P2PMove(CPoint(doorpt.x, doorpt.y - 20), aFish[B], aAction[B]);
        if (FishPt1.y < height / 2 - 10) {
            ++movement;
        }
    ...
    if (movement == 4) {
        std::cout << __LINE__ << "# 正在执行B鱼第五动作：向水池右上角移动并等待，准备进攻二号门" << std::endl;
    ...
    if (movement == 5) { // 在左上角等待
        std::cout << __LINE__ << "# 正在执行B鱼第六动作：向水池左上角移动并等待，准备进攻三号门" << std::endl;
    ...
    if (movement >= 2) {
        ...
        if (judgeball(aimpt) == 1) {
            ...
            if (judgearea(FishPt, 2)) //鱼在下平面
            {
                ...
                if (judgearea(aimpt, 2)) //球在下平面
                {
                    ...
                    {
                        //优先判断是否处于卡住状态
                        if (!judgearea(aimpt, 13) && judgearea(FishPt, 13) &&
                                (FishDir > -60 && FishDir < 60)) //鱼在第二个球门左面被卡住区域
                        {
                            std::cout << "一门左卡住" << FishDir << std::endl;
                            aAction[A].direction = 15;
                        } else if (aimpt.y > middle && aimpt.y < middledown &&
                                             aimpt.x > onedoorleft + 10 &&
                                             aimpt.x < onedoorright) //球在门口的时候
                        {
                            ...
                            if (FishPt.x < aimpt.x - 10) {
                                deletezero(10);
                                movingshot1(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(shotptup.x + 3, shotptup.y)); //对球最下面顶
                            }
                            if (FishPt.x > aimpt.x + 10) {
                                deletezero(10);
                                movingshot1(0, aimpt, aAction[A], A, aFish[A], doorpt,
                            ...
                            if (FishPt.x <= aimpt.x + 10 && FishPt.x >= aimpt.x - 10) {
                                deletezero(10);
                                movingshot1(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(shotptup.x, shotptup.y)); //对球最下面顶
                            }
                        } else if (aimpt.y > 350 && aimpt.y < 450 &&
                                             aimpt.x <
                                                     onedoorleft) ////在下左平面(靠下，在第一门左面）
                        {
                            deletezero(10);
                        ...
                        } else if (aimpt.y < 480 && aimpt.y > 450 &&
                                             aimpt.x <
                                                     onedoorright) ////在下左平面(靠下，在第一门左面）
                        {
                            deletezero(20);
                            std::cout << "在最下面" << std::endl;
                            movingshot2(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                    shotptup); //对球最下面顶
                        } else if (aimpt.y > middledown && aimpt.x > onedoorleft &&
                                             aimpt.x < onedoorright) ////在下左平面(在第一门之间）
                        {
                            deletezero(10);
                            std::cout << "在第一门之间" << std::endl;
                            if (FishPt.x > aimpt.x && FishPt.y < aimpt.y + 20) {
                                movingshot2(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(shotptup.x + 5, shotptup.y)); //对球最下面顶
                            } else {
                                movingshot1(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        shotpt0); //对球最下面顶
                                                                            // if(FishPt.x<aimpt.x-10)
                        ...
                        } else if (aimpt.y > middle && aimpt.y < middledown + 30 &&
                                             aimpt.x <
                                                     onedoorleft + 10) //在下左平面(靠上，在第一门左面)
                        {
                            deletezero(31);
                            if (aimpt.x < 1 * onedoorleft / 3) {
                                std::cout << "向右对球下面顶" << std::endl;
                                movingshot3(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(aimpt.x - 3, aimpt.y + 13));
                            } else if (aimpt.x < 2 * onedoorleft / 3) {
                                std::cout << "向右对球下面顶" << std::endl;
                                movingshot3(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(aimpt.x - 10, aimpt.y + 8));
                            } else {
                                std::cout << "向右对球最下面顶" << std::endl;
                                movingshot3(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(aimpt.x - 8, aimpt.y + 10));
                            }
                        } else {
                            deletezero(10);
                    ...
                    {
                        //优先判断是否处于卡住状态
                        if ((aimpt.y > middle + 50 && aimpt.y < middledown &&
                                 aimpt.x > twodoorleft && aimpt.x < twodoorright) &&
                                (FishDir > -60 && FishDir < 60)) //鱼在第二个球门左面被卡住区域
                        {
                            std::cout << "二门左卡住" << std::endl;
                            aAction[A].direction = 15;
                        } else if ((aimpt.y > middle && aimpt.x < twodoorright) &&
                                             (FishPt.y > middle + 50 && FishPt.y < middledown &&
                                                FishPt.x < twodoorright + 100 &&
                                                FishPt.x > twodoorright) &&
                                             (FishDir > 130 || FishDir < -130)) //球在二球门左,鱼在右
                        {
                            std::cout << "二门右卡住" << std::endl;
                            aAction[A].direction = 15;
                        } else if (aimpt.y > middle && aimpt.y < middledown + 20 &&
                                             aimpt.x < width / 2 && aimpt.x > onedoorright) //卡球区域
                        {
                            deletezero(20);
                            std::cout << "对着最上面击球" << std::endl;
                            movingshot2(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                    shotptdown); //对球最右面顶
                        } else if (aimpt.y > middledown + 20 && aimpt.x < width / 2 &&
                                             aimpt.x > onedoorright) //在第一球门右面击球区域
                        {
                            deletezero(20);
                            std::cout << "在第一球门右面击球区域" << std::endl;
                            if (aimpt.y > 350) {
                                movingshot2(
                                        0, aimpt, aAction[A], A, aFish[A], doorpt,
                                        CPoint(aimpt.x + radius, aimpt.y + 5)); //对球最右面顶
                            } else {
                                movingshot2(
                                        0, aimpt, aAction[A], A, aFish[A], doorpt,
                                        CPoint(aimpt.x + radius, aimpt.y + 8)); //对球最右面顶
                            }
                        } else if (aimpt.y > middle && aimpt.x >= onedoorright) {
                            deletezero(20);
                            std::cout << "正常区域" << std::endl;
                            movingshot2(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                    CPoint(aimpt.x + radius, aimpt.y)); //对球最右面顶
                        }
                    }
                }
            }
        ...
        if (judgeball(aimpt) == 2) {
            ...
            if (judgeareaup(FishPt, 1)) //鱼在上平面
            {
                ...
                if (judgeareaup(aimpt, 1)) //球在上平面
                {
                    ...
                    {
                        if (aimpt.y < middle &&
                                aimpt.y > middleup - 30) //在一门区域向左靠下
                        {
                            deletezero(21);
                            std::cout << "在一门区域向左靠下" << std::endl;
                            movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                    CPoint(aimpt.x - 10, aimpt.y + 3));
                        } else if (aimpt.y < middleup - 30 && aimpt.y > 100) {
                            deletezero(21);
                            std::cout << "一球门以左中间" << std::endl;
                            if (FishPt.y < aimpt.y) {
                                movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                        CPoint(aimpt.x - 10, aimpt.y - 5));
                            } else {
                                movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                        CPoint(aimpt.x - 10, aimpt.y + 5));
                            }
                            ...
                            movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                    CPoint(aimpt.x - 10, aimpt.y));
                        }
                    } else if (aimpt.y < middle && aimpt.x > onedoorright + 10 &&
                                         aimpt.x < twodoorleft) //一球门以右，二球门以左
                    {
                        if (aimpt.y < middle && aimpt.x > onedoorright &&
                                aimpt.x < twodoorleft && aimpt.y < 80) {
                            deletezero(21);
                            std::cout << "一门二门靠上" << std::endl;
                            movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                    CPoint(aimpt.x - 10, aimpt.y - 5));
                        } else if (aimpt.y < middle && aimpt.x > onedoorright &&
                                             aimpt.x < twodoorleft - 50 && aimpt.y < 160 &&
                                             aimpt.y >= 80) {
                            std::cout << "一门二门中间" << std::endl;
                            if (FishPt.x < aimpt.x && FishPt.y > aimpt.y - 60) {
                                deletezero(21);
                                movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                        CPoint(aimpt.x, aimpt.y - 10));
                            } else {
                                if (FishPt.x > aimpt.x) {
                                    deletezero(11);
                                    movingshot1(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                            CPoint(aimpt.x - 8, aimpt.y - 13));
                                } else {
                                    deletezero(11);
                                    movingshot1(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                            CPoint(aimpt.x - 8, aimpt.y - 8));
                                }
                            }
                        } else {
                            deletezero(21);
                            ...
                            if (aimpt.y > middle && aimpt.y < middle - 40) {
                                if (aimpt.x < twodoorleft - 20) {
                                    movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                            CPoint(aimpt.x - 8, aimpt.y - 5));
                                } else {
                                    movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                            CPoint(aimpt.x - 8, aimpt.y - 15));
                                }
                            } else {
                                if (aimpt.x < width / 2) {
                                    movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                            CPoint(aimpt.x - 8, aimpt.y - 12));
                                } else {
                                    movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                            CPoint(aimpt.x, aimpt.y - 12));
                                }
                            }
                        }
                    } else if (aimpt.y < middle && aimpt.x > twodoorleft &&
                                         aimpt.x < twodoorright) //两球门之间
                    {
                        ...
                        if (aimpt.y > 170) //已经在球门口了
                        {
                            if (FishPt.x < aimpt.x && FishPt.y > aimpt.y - 30) {
                                deletezero(21);
                                movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(aimpt.x - 5, aimpt.y - 16));
                            }
                            if (FishPt.x > aimpt.x && FishPt.y > aimpt.y - 30) {
                                deletezero(20);
                                movingshot2(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                ...
                                if (FishPt.x < aimpt.x - 10) {
                                    movingshot1(
                                            1, aimpt, aAction[A], A, aFish[A], doorpt,
                                            CPoint(shotptdown.x + 5, shotptdown.y)); //对球最下面顶
                                }
                                if (FishPt.x > aimpt.x + 10) {
                                    movingshot1(
                                            1, aimpt, aAction[A], A, aFish[A], doorpt,
                                ...
                                if (FishPt.x <= aimpt.x + 10 && FishPt.x >= aimpt.x - 10) {
                                    movingshot1(
                                            1, aimpt, aAction[A], A, aFish[A], doorpt,
                                            CPoint(shotptdown.x, shotptdown.y)); //对球最下面顶
                                }
                            }
                        } else if (aimpt.y < 60) {
                            deletezero(21);
                            movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                    CPoint(aimpt.x - radius, aimpt.y - 10));
                            /*if(FishPt.x<aimpt.x)
                            {
                            ...
                            if (FishPt.x <= aimpt.x + 10 && FishPt.x >= aimpt.x - 10) {
                                movingshot1(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        shotpt1); //对球最下面顶
                            }
                        }
                    } else if (aimpt.y < middle && aimpt.x > twodoorright) //二球门以右
                    {
                        if (aimpt.y < middle && aimpt.y > middleup - 30) //二球门右靠下
                        {
                            ...
                            if (FishPt.x > twodoorright + 20) {
                                ...
                                movingshot3(0, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                        CPoint(aimpt.x + 10, aimpt.y - 6)); //对球最右面顶
                            } else {
                                std::cout << "二球门右靠下偏左" << middleup << std::endl;
                                movingshot3(0, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                        CPoint(aimpt.x + 10, aimpt.y - 15)); //对球最右面顶
                            }
                        } else {
                            ...
                            if (aimpt.y < 80) {
                                ...
                                deletezero(21);
                                movingshot2(
                                        1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                        CPoint(aimpt.x - radius, aimpt.y - 8)); //对球最右面顶
                            } else {
                                std::cout << "往下往最上面偏右顶" << std::endl;
                                deletezero(11);
                                movingshot1(1, aimpt, aAction[A], A, aFish[A], doorpt1,
                                                        CPoint(aimpt.x + 3, aimpt.y - 10)); //对球最右面顶
                            }
                        }
                    } else {
                        movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt1,
            ...
            if (judgearea(FishPt, 7)) //鱼在下平面
            {
                ...
                if (FishPt.x > onedoorleft && (FishDir > 0 || FishDir < -120)) {
                    ...
                    aAction[A].speed = speedA;
                    aAction[A].direction = 15;
                } else if (FishPt.x > twodoorright + 30 && FishPt.y < doorpt1.y + 30 &&
                                     stop0 == 0 && stop1 == 0) {
        ...
        if (judgeball(aimpt) == 3) {
            ...
            if (stop0 == 0 && stop1 == 0 && judgearea(FishPt, 2)) //鱼在下平面
            {
                ...
                if (judgearea(aimpt, 2)) //球在下平面
                {
                    ...
                    {
                        //优先判断是否处于卡住状态
                        if (judgearea(FishPt, 13) &&
                                (FishDir > -60 && FishDir < 60)) //鱼在第二个球门左面被卡住区域
                        {
                            std::cout << "一门左卡住" << FishDir << std::endl;
                            aAction[A].direction = 15;
                        } else if (judgearea(aimpt, 18) && judgearea(FishPt, 14) &&
                                             (FishDir > 130 || FishDir < -130)) //球在一球门左,鱼在右
                        {
                            ...
                            movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                    CPoint(aimpt.x - radius, aimpt.y - 3)); //对球最下面顶
                        }

                    } else if (judgearea(aimpt, 19)) //球在一球门右
                    {
                        //优先判断是否处于卡住状态
                        if (judgearea(FishPt, 12) &&
                                (FishDir > -60 && FishDir < 60)) //鱼在第二个球门左面被卡住区域
                        {
                            std::cout << "二门左卡住" << std::endl;
                            aAction[A].direction = 15;
                        } else if (judgearea(aimpt, 16) && judgearea(FishPt, 15) &&
                                             (FishDir > 130 || FishDir < -130)) //球在二球门左,鱼在右
                        {
                            std::cout << "二门右卡住" << std::endl;
                            aAction[A].direction = 15;
                        } else if (aimpt.y > middle && aimpt.y < middledown + 10 &&
                                             aimpt.x < twodoorright && aimpt.x > onedoorright) {
                            deletezero(21);
                            ...
                            movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                    CPoint(aimpt.x - 10, aimpt.y - 13)); //对球最右面顶
                        } else if (aimpt.y > 430) {
                            deletezero(21);
                            std::cout << "球到下面" << std::endl;
                            movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                    CPoint(aimpt.x - 10, aimpt.y + 10));
                        }

                        else if (aimpt.x < twodoorright && aimpt.y < 440 &&
                                         aimpt.y > middledown + 10) {
                            deletezero(21);
                            std::cout << "击球区域" << std::endl;
                            if (FishPt.y < aimpt.x) {
                                movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(aimpt.x - 13, aimpt.y)); //对球最右面顶
                            } else {
                                movingshot2(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        shotpt3); //对球最右面顶
                            }
                        } else if (aimpt.y > doorpt3.y + (aChannel[2].width) / 2 - 7 &&
                                             aimpt.x > twodoorright + 30) {
                            deletezero(10);
                            ...
                            if (aimpt.x > doorpt3.x - 30) {
                                ...
                                movingshot1(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(aimpt.x + 5, aimpt.y - radius)); //对球右上顶
                            } else {
                                std::cout << "从下往上击球" << std::endl;
                                // movingshot1(0,aimpt,aAction[A],A,aFish[A],doorpt,shotpt3);//对球最右面顶
                                if (FishPt.x > aimpt.x) {
                                    movingshot1(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                            CPoint(aimpt.x - 13, aimpt.y + 8)); //对球最右面顶
                                } else {
                                    movingshot1(0, aimpt, aAction[A], A, aFish[A], doorpt,
                                                            shotpt3); //对球最右面顶
                                }
                                             ...
                                             aimpt.x > twodoorright) {
                            deletezero(21);
                            std::cout << "向右击球" << std::endl;
                            if (FishPt.y < aimpt.y) {
                                movingshot3(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(aimpt.x - 15, aimpt.y + 12)); //对球最右面顶
                            } else {
                                movingshot3(1, aimpt, aAction[A], A, aFish[A], doorpt,
                                                        CPoint(aimpt.x - 15, aimpt.y + 8)); //对球最右面顶
                            }
                        } else if (aimpt.y > doorpt3.y + (aChannel[2].width) / 2 - 7 &&
                                             aimpt.x < twodoorright + 30) {
                            deletezero(21);
            ...
            if (judgeareaup(FishPt, 4)) //鱼在上左平面
            {
                std::cout << "left fish 1->2" << middle << std::endl;
                spinP2PMove(doorpt, aFish[A], aAction[A]);
            }
            if (judgeareaup(FishPt, 5)) //鱼在上右平面
            {
                std::cout << "right fish 1->2" << std::endl;
                spinP2PMove(doorpt1, aFish[A], aAction[A]);
            }
        ...
        std::cout << __LINE__ << "# A鱼等待B鱼顶开一号球门" << std::endl;
        aAction[A].state = 0;
        aAction[A].mode  = 0;
        aAction[A].speed = 0;
    }
