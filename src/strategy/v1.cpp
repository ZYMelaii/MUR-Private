 ...
 * @brief 垂直向上，下顶球
 */
void StrategyCloneV1::movingshot1(int mode, CPoint aimpt, CFishAction &action,
...
void StrategyCloneV1::movingshot2(int mode, CPoint aimpt, CFishAction &action,
                                  int fishID, CFishInfo m_FishInfo,
                                  CPoint doorpt,
...
void StrategyCloneV1::movingshot3(int mode, CPoint aimpt, CFishAction &action,
                                  int fishID, CFishInfo m_FishInfo,
                                  CPoint doorpt,
                                  CPoint shotpt0) { //垂直向左，向右顶球
  CPoint FishPt = m_FishInfo.centerPos();           // 鱼身坐标
 ...
 * @note 版本：最终版2 红4 - 改
 */
bool StrategyCloneV1::Strategy(RefArray<CFishAction> aAction,
  ...
  */
  ...
  std::cout << __LINE__ << "# A鱼顶左面球门策略-水球区域标号@judgeball: "
            << judgeball(aimpt) << "\n";
  if (movement == 0) {
    std::cout << __LINE__ << "# 正在执行B鱼第一动作" << std::endl;
  ...
  if (movement == 1) {
    std::cout << __LINE__ << "# 正在执行B鱼第二动作" << std::endl;
  ...
  if (movement == 2) {
    std::cout << __LINE__ << "# 正在执行B鱼第三动作" << std::endl;
  ...
  if (movement == 3) {
    std::cout << __LINE__ << "# 正在执行B鱼第四动作" << std::endl;
    P2PMove(CPoint(doorpt.x, doorpt.y - 20), aFish[B], aAction[B]);
    if (FishPt1.y < height / 2 - 10) {
      ++movement;
    }
  ...
  if (movement == 4) {
    std::cout << __LINE__ << "# 正在执行B鱼第五动作" << std::endl;
  ...
  if (movement == 5) { // 在左上角等待
    std::cout << __LINE__ << "# 正在执行B鱼第六动作：等待3门" << std::endl;
